...
<body>
	...
	<h3>* 동등비교 연산</h3>
	- s1의 데이터와 s2의 데이터 값이 일치하는가? ${ s1 == s2 } <br>
	- s2와 s4의 값이 일치하는가? ${ s2 == s4 } 또는 ${ s2 eq s4 }<br>
	<%-- EL에서는 문자열에 대한 동등비교가 자바에서의 equals와 같이 동작됨!! --%>
	
	- s2와 s3의 값이 일치하지 않는가? ${ s2 != s3 } 또는 ${ s2 ne s3 } <br>
	
	- small에 담긴값이 10인가? ${ small == 10 } 또는 ${ small eq 10 } <br>
	
	- s3에 담긴값이 "힘내세요!!!"와 일치하는가? ${ s3 == "성준님 화이팅!!" } 또는 ${ s3 eq "성준님 화이팅!!" } <br>
	- s3에 담긴값이 "힘내세요!!!"와 일치하는가? ${ s3 == '성준님 화이팅!!' } 또는 ${ s3 eq '성준님 화이팅!!' } <br>
	<%-- EL 에서는 문자열 리터럴 표현 시 작은따옴표, 큰따옴표 상관없이 사용가능!! --%>
	...
	- student2가 null인가? ${ student2 == null } 또는 ${ empty Student2 } <br>
	...
	+ big이 small보다 크고 list1이 비어있는가? : ${ big > small && empty list1 }
