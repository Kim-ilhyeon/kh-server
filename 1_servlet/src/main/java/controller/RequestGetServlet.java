...
//		System.out.println("@@@@@ Get 요청이 들어옴 @@@@@");
		 ...
		 * ------------------------------------
		 * 
		 * - 요청 시 전달된 데이터 추출(request 객체의 parameter 영역 안에 데이터가 담겨져 있음)
		 *   request.getParameter("키값"):String -> 키 값에 해당하는 데이터를 반환
		 * 
		 */
		String userName = request.getParameter("userName");		// 이름(userName)
		// input:text --> "입력된 값" 또는 ""
		String gender = request.getParameter("gender");			// 성별(gender)
		// input:radio --> "선택된 값" 또는 null (선택항목이 없을 경우)
		...
		// => request.getParameterValues("키값"):String[] -> 키 값에 해당하는 데이터를 배열로 반환
		String[] foods = request.getParameterValues("food");				// 좋아하는 음식(food)
		// input:checkbox --> "[선택된 값(, 선택된 값)]" 또는 null (선택항목이 없을 경우)
//		System.out.println(Arrays.toString(foods));
		...
		pw.printf("<span>%s</span>에 살고 있습니다.<br>", address);		
